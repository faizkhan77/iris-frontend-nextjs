...
// Assuming these types are defined in a shared file as per your original React setup
import type {
...
// --- Sub-Components (Styling updated to match old Next.js version) ---
  ...
  return (
    <div className="rounded-lg border border-element-border bg-element-bg p-3">
      ...
      <div className="relative h-2 w-full rounded-full overflow-hidden bg-background">
        ...
        {/* Changed pointer to white to match old design */}
        <motion.div
...
const HistoricalPerformance = ({ data }: HistoricalPerformanceProps) => {
  if (!data) return null;
  // Set default state to false to be collapsed initially, as in the old code
...
export function TechnicalSummaryCard({
  ...
  // The main layout is now re-ordered and re-styled to match the old Next.js component
  return (
    <div className="w-full text-sm">
      <h2 className="text-lg font-semibold text-text-primary mb-3">{title}</h2>
      <p className="text-text-secondary mb-4">{data.summaryText}</p>

      {/* Correct Order: Historical Performance is now here */}
            ...
            >
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 py-3">
                {data.detailsTable.map((item, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center text-xs border-b border-element-bg pb-2 last:border-b-0"
                  >
