...
// Import the REAL child components and their data types
...
import type { FundamentalAnalysisCardProps } from "./FundamentalAnalysisCard";
import type { TechnicalSummaryData } from "./constant";
import type { SentimentAnalysisData } from "./SentimentAnalysisCard";

// --- Data Structures (from Next.js reference) ---
type TabData =
  | {
      label: "Fundamentals";
      verdict: string;
      data: FundamentalAnalysisCardProps["data"];
    }
...
export function CrossAgentAnalysisCard({
  ...
  // Initialize state from the incoming data prop
  const [activeTab, setActiveTab] = useState(data?.tabs?.[0]?.label);

  if (!data || !data.tabs) {
    return (
      <div className="text-text-secondary">
        Missing data for cross-agent analysis.
      </div>
    );
  }
  ...
  const renderActiveTabContent = () => {
    ...
    // Render the REAL components with their respective data
